...
function makeRSS (user, tweets) {
  ...
  for (i = 0; i < tweets.length; i++) {
    ...
    if (!t)
      continue;
    rss += "<item>\n\t<title><![CDATA["
    // Note: removed as no need for the author name to appear in the title. There is the author field for that!
//           + t.authorTwitterName
//           + ' '
           + t.tweetHTML
           + "]]></title>\n\t<author><![CDATA["
...
function extractTweets (tweets, max) {
  ...
  for (i = 0; i < max; i++) {
    if (tweets.div[i]) {
      ...
      var metadiv;
      var rt;
      if (!header) // no tweet but probably a list of followers 
        continue;
      ...
      else if (body.p[1] && body.p[1].content) // newer style commented re-tweet (should look for class=="ProfileTweet-text js-tweet-text u-dir")
      ...
      if (tweetLinks) {
        for (j = 0; j < tweetLinks.length; j++) {
          ...
          // TODO: two whitespaces is not always working. But do not know of a better heuristic where to insert the tweets.
